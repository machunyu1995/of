		...
		for (var CurRange = 0; CurRange < RangesCount; CurRange++)
		{
			...
			while (null != Element)
			{
				if (!pGraphics.DrawTextArtComment)
				{
				    ...
				    var DocumentComments = this.LogicDocument.Comments;
                    var CommentIds = Element.Additional.CommentId
                    var SysCommentIds = DocumentComments.Get_System_Comments();
                    if(CommentIds.some(value => !SysCommentIds.includes(value))){
                         // 包含非系统批注
                        if (Element.Additional.Active === true)
                            pGraphics.b_color1(240, 200, 120, 255);
                        else {
                            pGraphics.b_color1(246, 234, 209, 255);
                        }
                        pGraphics.rect(Element.x0, Element.y0, Element.x1 - Element.x0, Element.y1 - Element.y0);
                        pGraphics.df();
                    }
                    if(CommentIds.some(value => SysCommentIds.includes(value))){
                         // 包含系统批注
                        sysCommentElements.push(Element)
                    }
            ...
            sysCommentElements.sort((a,b)=>a.x0 - b.x0)
            for(let i =0;i<sysCommentElements.length;i++){
                let Element = sysCommentElements[i];
                if (Element.Additional.Active === true){
                  pGraphics.p_color(255, 0, 0, 255);
                  pGraphics.p_width(500);
                }
                else {
                    pGraphics.p_color(246, 234, 209, 0);
                }
                let left = true;
                let right = true;
                if (i>0){
                    let pre = sysCommentElements[i-1];
                    if(pre.x1 == Element.x0){
                        left = false;
                    }
                }
                if(i < sysCommentElements.length - 1){
                    let next = sysCommentElements[i+1];
                    if(Element.x1 == next.x0){
                        right = false;
                    }
                }

                pGraphics.rectByLine(Element.x0, Element.y0 - 0.5, Element.x1 - Element.x0, Element.y1 - Element.y0 + 1, left, right);
                pGraphics.ds();
            }
