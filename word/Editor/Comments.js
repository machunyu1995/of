...
//-----------------------------------------------------------------------------------
	...
	{
		console.log('CComments.prototype.GetAllComments ')
		console.log(this.m_arrCommentsById)
		return this.m_arrCommentsById;
	};

	 CComments.prototype.Get_System_Comments = function()
    {
        var systemCommentIds = [];
		for (var id in this.m_arrCommentsById) {
			var comment = this.m_arrCommentsById[id];
			var userName = comment.Data.m_sInitials;
			try {
				var parsed = JSON.parse(userName);
				if (typeof parsed === 'object' && parsed !== null) {
					systemCommentIds.push(id);
				}
			} catch (e) {
				// 不是JSON格式的userName则跳过
				continue;
			}
		}
		return systemCommentIds;
    };

	CComments.prototype.GetCommentsListById = function(type)
	{
		// 获取当前文档ID
		var currentDocId = editor?.DocInfo?.get_Id();
		console.log('GetCommentsListById:', type, 'currentDocId:', currentDocId, this.m_arrCommentsById);

		var simplifiedComments = [];
		for (var id in this.m_arrCommentsById) {
			var comment = this.m_arrCommentsById[id];
			var userName = comment.Data.m_sInitials;
		...
		try {
			...
			window.parent.postMessage({
				type: 'GetCommentsListResponseBy' + type,
				data: simplifiedComments,
				key: currentDocId  // 添加文档ID
			}, '*');
			console.log('Comments list sent successfully for document:', currentDocId);
		} catch (e) {
			console.error('Error sending comments list:', e);
		}
	};
