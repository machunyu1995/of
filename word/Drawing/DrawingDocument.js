...
function CDrawingDocument()
{
	...
	this.UpdateTarget = function (x, y, pageIndex, scrollMode = true)
	{
		console.log('UpdateTarget', scrollMode);
		...
		if (scrollMode === false) {
			...
			// 新逻辑：让目标居中显示
			var screenCenterY = (boxY + boxB) / 2;  // 屏幕中心Y坐标
			var targetCenterY = pos.Y + targetSizePx / 2;  // 目标中心Y坐标
			
			// 计算让目标居中需要的滚动距离
			nValueScrollVer = targetCenterY - screenCenterY;
			
			// 死区设置：如果偏移小于30像素，就不滚动（避免频繁小幅移动）
			var deadZone = 30;
			if (Math.abs(nValueScrollVer) < deadZone) {
				nValueScrollVer = 0;
			}
		}
